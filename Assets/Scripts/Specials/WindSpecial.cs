...
public class WindSpecial : MonoBehaviour {

    private static WindSpecial instance = null;

    public List<WindTurret> turrets;
    public Image recharge;
    public TextMeshProUGUI num;

    void Awake() {
        instance = this;
        turrets = new List<WindTurret>();
    ...
    public static void AddNewWindTurret(WindTurret wt) {
        instance.turrets.Add(wt);
    }

    public static void RemoveTurret(WindTurret wt) {
        if(!instance)
            return;

        if(instance.turrets.Count > 0)
            instance.turrets.Remove(wt);
